...
export class FpStore<S> {
  ...
  dispatch<T extends Action>(action: T): Observable<T> {
    return of(action).pipe(tap(a => this.store.dispatch(a)))
  }

  dispatchRequest<T extends RequestAction, K>(
    action: T,
    Success: ActionClass<ResponseAction<K>>,
    Error?: ActionClass<ResponseAction<any>>,
  ): Observable<K> {
    const isResponseAction = (requestId: number) => (a: any) =>
      (<ResponseAction<any>>a).responseId === requestId
        ...
        merge(
          this.action$.pipe(
            ...
            flatMap(_a => {
              ...
              return throwError(
                `[dispatchRequest] unrecognized action ${_a.toString()}`,
              )
            }),
          ),
          defer(() => {
